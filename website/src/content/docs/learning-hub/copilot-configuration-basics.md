...
**Session-only model change** *(v1.0.72+)*: Use `/model --session` (or `/model -s`) to change the model, reasoning effort, or context window **for just the current session**, leaving your global and repository settings unchanged:

```
/model --session          # open picker for session-only changes
/model -s                 # shorthand
```

This is useful when you want to try a different model for a specific task without affecting your default configuration or the repository's pinned model. Session-scoped changes are discarded when the session ends.

**Available models**: GitHub Copilot CLI supports models from multiple providers. As of v1.0.70+, **GPT-5.6** is available as a supported model in addition to the existing Claude, GPT-4, and Gemini families. Browse the full list in `/model`.
