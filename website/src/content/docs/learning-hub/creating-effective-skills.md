...
*(v1.0.72+)* You can also install skills through the `copilot plugins` commands, which is useful when a skill is distributed as a standalone package or URL:

```bash
copilot plugins install --skill ./my-skill/       # install from a local path
...
copilot plugins remove --skill my-skill           # remove a skill installed via plugins
```
