...
Yes — **multi-turn subagents are always enabled** (v1.0.72+). You can send follow-up messages to subagents while they are still running, allowing you to steer or provide additional context mid-task without waiting for them to finish. This makes interactive orchestration much more responsive for long-running delegated work.
