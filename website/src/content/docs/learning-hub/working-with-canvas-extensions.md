...
description: 'Create and iterate on GitHub Copilot app canvases using /create-canvas, then shape them into reusable project or personal extensions. Also available in Copilot CLI (v1.0.71+).'
authors:
...
> **CLI support (v1.0.71+)**: Canvas extensions are now also supported in **GitHub Copilot CLI**. If your extension uses the Copilot SDK's canvas APIs, it can render and receive interactions inside a CLI session — not just the Copilot app. The authoring model is the same; the CLI opens the canvas panel when your extension calls `createCanvas`.
