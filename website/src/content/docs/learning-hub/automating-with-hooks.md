...
> **Loop protection (v1.0.72+)**: If an `agentStop` hook always exits non-zero (always blocks), the CLI will no longer loop indefinitely. After **8 consecutive blocks**, the CLI forces the turn to end and passes a `stop_hook_active: true` flag in the JSON input to subsequent hook invocations. Your hook can inspect this flag and self-limit — for example, by allowing the turn to end gracefully after repeated failures rather than looping forever. This prevents runaway scenarios where a failing hook stalls the agent permanently.
>
> Example check in a hook script:
> ```bash
> INPUT=$(cat)
> if echo "$INPUT" | jq -e '.stop_hook_active == true' > /dev/null 2>&1; then
>   # Force-end is active; exit 0 to let the turn complete
>   exit 0
> fi
> # Normal lint check
